function dydt = dynamic_model(t, y, M_hat, C_hat, G_hat, tau_f, tau)
    ...
    assert(all(size(tau_f) == [3, 1]), 'tau_f must be a 3x1 vector');
    assert(all(size(tau) == [3, 1]), 'tau must be a 3x1 vector');

    % Compute joint accelerations
    q_ddot = M_hat \ (tau - C_hat .* q_dot - G_hat - tau_f);
